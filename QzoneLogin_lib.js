...
$.Encryption = $pt.Encryption = function() {
    ...
    function core_hmac_md5(t, e) {
        ...
        for (var n = Array(16), o = Array(16), p = 0; 16 > p; p++) n[p] = 909522486 ^ i[p], o[p] = 1549556828 ^ i[p];
    ...
    function binl2b64(t) {
        for (var e = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/", i = "", n = 0; n < 4 * t.length; n += 3)
            for (var o = (t[n >> 2] >> 8 * (n % 4) & 255) << 16 | (t[n + 1 >> 2] >> 8 * ((n + 1) % 4) & 255) << 8 | t[n + 2 >> 2] >> 8 * ((n + 2) % 4) & 255, p = 0; 4 > p; p++) i += 8 * n + 6 * p > 32 * t.length ? b64pad : e.charAt(o >> 6 * (3 - p) & 63);
        return i
    }
    ...
    function getEncryption(t, e, i, n) {
        i = i || "", t = t || "";
        for (var o = n ? t : md5(t), p = hexchar2bin(o), r = md5(p + e), s = TEA.strToBytes(i.toUpperCase(), !0), l = Number(s.length / 2).toString(16); l.length < 4;) l = "0" + l;
        TEA.initkey(r);
        var a = TEA.encrypt(o + TEA.strToBytes(e) + l + s);
        TEA.initkey("");
        for (var c = Number(a.length / 2).toString(16); c.length < 4;) c = "0" + c;
        var u = $pt.RSA.rsa_encrypt(hexchar2bin(c + a));
        return setTimeout(function() {
            __monitor(488358, 1)
        }, 0), btoa(hexchar2bin(u)).replace(/[\/\+=]/g, function(t) {
            return {
                "/": "-",
                "+": "*",
                "=": "_"
...
$.RSA = $pt.RSA = function() {
    ...
    function $(t) {
        ...
        if (r-- > 0)
            for (s < this.DB && (i = this[r] >> s) > 0 && (o = !0, p = u(i)); r >= 0;) e > s ? (i = (this[r] & (1 << s) - 1) << e - s, i |= this[--r] >> (s += this.DB - e)) : (i = this[r] >> (s -= e) & n, 0 >= s && (s += this.DB, --r)), i > 0 && (o = !0), o && (p += u(i));
        return o ? p : "0"
    }
            ...
            if (0 != g) {
                ...
                for (; --$ >= 0;) {
                    ...
                    if ((i[_] += p.am(0, w, i, $, 0, u)) < w)
                        for (p.dlShiftTo($, v), i.subTo(v, i); i[_] < --w;) i.subTo(v, i)
                }
    ...
    function W(t, e) {
        var i;
        return i = 256 > t || e.isEven() ? new P(e) : new B(e), this.exp(t, i)
    }
    ...
    (r.prototype.am = c, pt = 28), r.prototype.DB = pt, r.prototype.DM = (1 << pt) - 1, r.prototype.DV = 1 << pt;
    ...
    var at, ct, ut = "0123456789abcdefghijklmnopqrstuvwxyz",
        gt = new Array;
    for (at = "0".charCodeAt(0), ct = 0; 9 >= ct; ++ct) gt[at++] = ct;
    for (at = "a".charCodeAt(0), ct = 10; 36 > ct; ++ct) gt[at++] = ct;
    for (at = "A".charCodeAt(0), ct = 10; 36 > ct; ++ct) gt[at++] = ct;
    P.prototype.convert = Q, P.prototype.revert = I, P.prototype.reduce = H, P.prototype.mulTo = M, P.prototype.sqrTo = V, B.prototype.convert = U, B.prototype.revert = O, B.prototype.reduce = j, B.prototype.mulTo = F, B.prototype.sqrTo = R, r.prototype.copyTo = d, r.prototype.fromInt = h, r.prototype.fromString = m, r.prototype.clamp = _, r.prototype.dlShiftTo = q, r.prototype.drShiftTo = S, r.prototype.lShiftTo = C, r.prototype.rShiftTo = T, r.prototype.subTo = x, r.prototype.multiplyTo = A, r.prototype.squareTo = N, r.prototype.divRemTo = E, r.prototype.invDigit = D, r.prototype.isEven = z, r.prototype.exp = G, r.prototype.toString = $, r.prototype.negate = v, r.prototype.abs = w, r.prototype.compareTo = y, r.prototype.bitLength = k, r.prototype.mod = L, r.prototype.modPowInt = W, r.ZERO = f(0), r.ONE = f(1);
    var dt, ht, ft;
    if (null == ht) {
        ht = new Array, ft = 0;
        var mt;
        // if ("Netscape" == navigator.appName && navigator.appVersion < "5" && global.crypto && global.crypto.random) {
        //     var _t = global.crypto.random(32);
        //     for (mt = 0; mt < _t.length; ++mt) ht[ft++] = 255 & _t.charCodeAt(mt)
        // }
        for (; $t > ft;) mt = Math.floor(65536 * Math.random()), ht[ft++] = mt >>> 8, ht[ft++] = 255 & mt;
        ft = 0, Z()
    }
...
(function(t) {
    ...
    function l(t) {
        _ = new Array(8), $ = new Array(8), v = w = 0, k = !0, m = 0;
        var i = t.length,
            n = 0;
        ...
        for (n = 1; 2 >= n;) 8 > m && (_[m++] = 255 & e(), n++), 8 == m && c();
        for (var o = 0; i > 0;) 8 > m && (_[m++] = t[o++], i--), 8 == m && c();
        for (n = 1; 7 >= n;) 8 > m && (_[m++] = 0, n++), 8 == m && c();
        return y
    }
    ...
    function c() {
        ...
        for (var e = u(_), t = 0; 8 > t; t++) y[v + t] = e[t] ^ $[t], $[t] = _[t];
        w = v, v += 8, m = 0, k = !1
    }

    function u(t) {
        for (var e = 16, o = i(t, 0, 4), p = i(t, 4, 4), r = i(f, 0, 4), s = i(f, 4, 4), l = i(f, 8, 4), a = i(f, 12, 4), c = 0, u = 2654435769; e-- > 0;) c += u, c = (4294967295 & c) >>> 0, o += (p << 4) + r ^ p + c ^ (p >>> 5) + s, o = (4294967295 & o) >>> 0, p += (o << 4) + l ^ o + c ^ (o >>> 5) + a, p = (4294967295 & p) >>> 0;
        var g = new Array(8);
        return n(g, 0, o), n(g, 4, p), g
    }

    function g(t) {
        for (var e = 16, o = i(t, 0, 4), p = i(t, 4, 4), r = i(f, 0, 4), s = i(f, 4, 4), l = i(f, 8, 4), a = i(f, 12, 4), c = 3816266640, u = 2654435769; e-- > 0;) p -= (o << 4) + l ^ o + c ^ (o >>> 5) + a, p = (4294967295 & p) >>> 0, o -= (p << 4) + r ^ p + c ^ (p >>> 5) + s, o = (4294967295 & o) >>> 0, c -= u, c = (4294967295 & c) >>> 0;
        var g = new Array(8);
        return n(g, 0, o), n(g, 4, p), g
    }
    ...
    t.TEA = {
        ...
        enAsBase64: function(t, e) {
            for (var i = h(t, e), n = l(i), o = "", p = 0; p < n.length; p++) o += String.fromCharCode(n[p]);
            return btoa(o)
        },
        decrypt: function(t) {
            var e = h(t, !1),
    ...
    q.PADCHAR = "=", q.ALPHA = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/", q.getbyte = function(t, e) {
        ...
        for (e = 0; s > e; e += 3) i = p(t, e) << 16 | p(t, e + 1) << 8 | p(t, e + 2), r.push(o.charAt(i >> 18)), r.push(o.charAt(i >> 12 & 63)), r.push(o.charAt(i >> 6 & 63)), r.push(o.charAt(63 & i));
        switch (t.length - s) {
            case 1:
        ...
        return r.join("")
    }, global.btoa || (global.btoa = q.encode)
})(global)
